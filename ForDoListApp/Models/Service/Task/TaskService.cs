...
namespace Models.Service.Task
{
    public class TaskService : ITaskService
    {
        ...
        public void SaveTask(TaskEntity task)
        {
            try
            {
                Console.WriteLine($"Saving task: Title={task.TaskTitle}, UserId={task.UserId}, TaskId={task.TaskId}");
                // Ensure UserId exists in Users table
                var userExists = _context.Users.Any(u => u.UserId == task.UserId);
                if (!userExists)
                {
                    Console.WriteLine($"Error: UserId={task.UserId} does not exist in Users table");
                    throw new InvalidOperationException($"UserId {task.UserId} does not exist");
                }
                ...
                Console.WriteLine($"Task saved: TaskId={task.TaskId}, RowsAffected={rowsAffected}");
            }
            catch (Exception ex)
            {
